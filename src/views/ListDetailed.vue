                    ...
                    <n-grid :y-gap="10" :cols="1" style="width: 671px; height: 256px">
                        <n-gi style="height: 50px">
                            <n-grid>
                                ...
                                <n-gi v-if="this.$cookies.get('userid') == this.playlist.up.id" :span="3">
                                    <n-button quaternary :focusable="false" @click="editList">
                                        <n-icon>
                                            <CreateOutline />
                                        </n-icon>
                        ...
                        <n-gi>
                            <div>
                                <a v-if="this.$cookies.get('userid') == this.playlist.up.id && this.tagsNum == 0
                        ...
                        <n-gi>
                            <div v-if="this.playlist.shared === true">歌单简介：{{ this.playlist.profile }}</div>
                            <div v-else>收藏夹简介：{{ this.playlist.profile }}</div>
                        </n-gi>
                        <n-gi>
                            <n-button :focusable="false" @click="playAll" class="playall-button">
                                <n-icon>
                            ...
                            <n-button v-if="this.$cookies.get('userid') == this.playlist.up.id && this.playlist.shared === false
                            ...
                            <n-button v-if="(this.$cookies.get('userid') == this.playlist.up.id ||
                                this.$cookies.get('is_superuser') == 'true') &&
                                this.playlist.shared === true
                            ...
                            <n-button v-if="this.$cookies.isKey('userid') &&
                                this.$cookies.get('userid') != this.playlist.up.id
    ...
    <div class="music-list-item" v-if="this.playlist.up !== undefined">
        <n-grid>
            ...
            <n-gi :span="16">
                <a-divider style="height: 1.8px; background-color: #dddddd" />
                <list-table v-if="playlist.music_set.length > 0" :view-mode="'user'" :position="'CollectionView'"
                    v-model:songData="playlist.music_set" :currentListId="this.playlist.id"></list-table>
                <div v-else :style="{
                    'text-align': 'center',
                    'margin': '20px'
                }">没有歌曲...</div>
        ...
        <div>
            <n-card class="edit-list-hodder" style="--n-border-radius: 20px">
                <n-grid>
                    ...
                    <n-gi :span="22">
                        <span class="modify-title">
                            <div class="edit-list-title">编辑{{ playlist.shared ? '歌单' : '收藏夹' }}信息</div>
                        </span>
                    </n-gi>
                    <n-gi :span="1">
                        <div class="close-edit-modify">
