...
export default {
  ...
  emits: ["clickUpload", "flushCollections", "updateCollections"],
};
</script>
<template>
  <!-- 更新所有收藏夹信息是给上一级用的 -->
  <create-new-collection-view
    v-if="['Collection', 'CollectionView'].includes(position)"
    :show="showModal"
    @flushCollections="$emit('flushCollections')"
    @closeCreateWindow="showModal = false"
  ></create-new-collection-view>
  <div
    class="image_table"
    :style="{
  ...
  >
    ...
    <div
      class="list_top_nav"
      v-if="['Collection', 'UploadedSongs', 'CollectionView'].includes(position)"
    >
      <n-popover
        trigger="hover"
        :style="{
      ...
      >
        <template #trigger>
          ...
          >
            ...
            <n-icon
              :size="25"
              v-if="position === 'Collection' || position === 'CollectionView'"
            >
    ...
    <div class="image_table_list" v-if="rows.length > 0">
      <template v-for="data in currentPageData" :key="data.Key">
        <image-table-entry
          ...
          @clickEntry="handleClickEntry"
          @deleteCollection="handleClickDeleteCollection"
          @shareCollection="handleClickShareCollection"
          @cancelShare="handleClickCancelShare"
          @deleteUploadedSong="handleClickDeleteUploadedSongs"
          :EntrySize="entrySize"
          :Edit="['Collection', 'Songlist', 'UploadedSongs'].includes(position)"
        ></image-table-entry>
      </template>
      <div class="pagination_box">
        <n-pagination
          :page-count="pageCount"
          v-model:page-size="pageArgs.pageSize"
          v-model:page="pageArgs.currentPage"
          show-quick-jumper
          show-size-picker
          :default-page-size="1"
          :page-sizes="[
    ...
    <div
      v-else
      :style="{ color: getRGBString(fontColorString) }"
      class="no_content_notice"
    >
      {{
